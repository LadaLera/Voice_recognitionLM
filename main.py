"""
Игра для тренировки произношения с определением акцента
"""
import os
...
# Импорт нашего модуля детекции акцента
try:
    from accent_detector_claude import detect_accent
...
# Список слов для игры (расширенный)
WORDS = [
    # Животные
    "cat", "dog", "lion", "tiger", "elephant", "giraffe", "monkey", "bear",
    "fox", "wolf", "rabbit", "mouse", "bird", "fish", "snake", "frog",
    
    # Природа
    "tree", "flower", "river", "mountain", "ocean", "lake", "sky", "cloud",
    "sun", "moon", "star", "rain", "snow", "wind", "forest", "desert",
    
    # Эмоции
    "happy", "sad", "angry", "excited", "scared", "calm", "proud", "brave",
    "kind", "tired", "bored", "nervous", "worried", "surprised", "confused",
    
    # Еда
    "apple", "banana", "orange", "bread", "cheese", "milk", "water", "juice",
    "pizza", "burger", "chicken", "fish", "rice", "cake", "chocolate",
    
    # Мебель
    "chair", "table", "bed", "lamp", "door", "window", "mirror", "carpet",
    
    # Действия
    "run", "walk", "jump", "sit", "stand", "talk", "listen", "read", "write",
    "sing", "dance", "eat", "drink", "sleep", "play", "think", "swim",
    
    # Транспорт
    "car", "bus", "train", "plane", "bike", "boat", "taxi", "truck",
...
class SpeechRecognizer:
    """Класс для распознавания речи БЕЗ зависаний"""
    ...
    def _recognize_thread(self):
        """Поток для распознавания (не блокирует UI)"""
        try:
...
class GameState:
    ...
    def mark_wrong(self, recognized: str, accent: str):
        """Отметка неправильного ответа"""
        self.last_accent = accent
        
        # ИСПРАВЛЕНО: Более понятные сообщения об ошибках
...
# ============= ИСПРАВЛЕННАЯ ФУНКЦИЯ ИГРОВОГО ЭКРАНА =============
def draw_game_screen(state):
    """Отрисовка игрового экрана"""
    screen.fill(state.background_color)
    
    # Текущее слово (крупно и по центру)
    ...
    # ИСПРАВЛЕНО: Сообщение ниже
    message_y = HEIGHT // 2 + 40  # Было HEIGHT // 2 + 20
...
# ============= ОБНОВЛЕННАЯ ГЛАВНАЯ ИГРА =============
def main_game():
    """Основная игра БЕЗ зависаний"""
    state = GameState()
    ...
    while running:
        clock.tick(60)  # 60 FPS - плавная анимация
...
# ============= РЕЖИМ ОПРЕДЕЛЕНИЯ АКЦЕНТА БЕЗ ЗАВИСАНИЙ =============
def detect_accent_mode():
    """Режим определения акцента"""
    recognizer = SpeechRecognizer()
    result_text = ""
    waiting_for_result = False
...
# ============= ИСПРАВЛЕННАЯ ФУНКЦИЯ ГЛАВНОГО МЕНЮ =============
def main_menu():
    """Главное меню"""
    clock = pygame.time.Clock()
    running = True
    ...
    while running:
        ...
        # ИСПРАВЛЕНО: Кнопки со сдвигом вниз
        button_start_y = HEIGHT // 2 + 20  # Начало кнопок ниже
        button_spacing = 70  # Расстояние между кнопками
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if button_game.collidepoint(event.pos):
                    ...
                    # ИСПРАВЛЕНО: Проверка доступности переводчика
